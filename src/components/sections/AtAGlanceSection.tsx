  ...
  return (
    ...
    >
      ...
      <div className={`${styles.card} ${isVisible ? styles.visible : ''}`}>
        <h2 className={styles.heading}>{t(config.heading || 'A Weekend in Singapore')}</h2>
        <div className={styles.divider} />
        
        <div className={styles.detailsGrid}>
          {config.date     && <div className={styles.row}><span>{t(config.date)}</span></div>}
          {config.venueText && <div className={styles.row}><span>{t(config.venueText)}</span></div>}
          {config.roomText  && <div className={styles.row}><span>{t(config.roomText)}</span></div>}
          {config.dressCode && <div className={styles.row}><span>{t(config.dressCode)}</span></div>}
          {rsvpText && <div className={styles.row}><span className={styles.highlight}>{t(rsvpText)}</span></div>}
        </div>
      </div>
    </section>
  );
